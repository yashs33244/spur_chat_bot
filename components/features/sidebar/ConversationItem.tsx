...
export function ConversationItem({
  ...
  return (
    ...
    >
      ...
      {!isEditing && !isSaving && (
        // Mobile: always visible. Desktop/tablet: hover only.
        <div className="flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
          <button
            title="Rename"
            onClick={(e) => { e.stopPropagation(); setIsEditing(true); }}
            className="h-7 w-7 flex items-center justify-center rounded-lg text-neutral-500 hover:text-neutral-200 active:scale-95 transition-colors"
          >
          ...
          <button
            ...
            className="h-7 w-7 flex items-center justify-center rounded-lg text-neutral-500 hover:text-red-400 active:scale-95 transition-colors"
          >
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
